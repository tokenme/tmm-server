...
type TokenPrice struct {
	Rate            decimal.Decimal `json:"rate"`     // current rate
	Currency        string          `json:"currency"` // token price currency (USD)
	Diff            decimal.Decimal `json:"diff"`     // 24 hour rate difference (in percent)
	Diff7d          decimal.Decimal `json:"diff7d"`
	Ts              int64           `json:"ts,string"` // last rate update timestamp
	MarketCapUsd    decimal.Decimal `json:"marketCapUsd"`
	AvailableSupply decimal.Decimal `json:"availableSupply"`
...
type Token struct {
	...
	Decimals       decimal.Decimal `json:"decimals"`       // number of significant digits
	Price          TokenPrice      `json:"price"`          // token price (false, if not available)
