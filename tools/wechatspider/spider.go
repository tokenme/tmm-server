...
func (this *Crawler) getGzh() ([]string, error) {
	db := this.service.Db
	rows, _, err := db.Query(`SELECT name FROM tmm.wx_gzh WHERE updated_at IS NULL OR updated_at<DATE_SUB(NOW(), INTERVAL 12 HOUR)`)
	if err != nil {
		return nil, err
	}
	var names []string
...
func (this *Crawler) Publish() error {
	...
	for _, row := range rows {
		...
		val = append(val, fmt.Sprintf("(0, '%s', '%s', '%s', '%s', 500, 500, 5, 20)", db.Escape(title), db.Escape(digest), db.Escape(link), db.Escape(cover)))
	}
	if len(val) > 0 {
