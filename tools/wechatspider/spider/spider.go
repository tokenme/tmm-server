...
func New(slackBot *Slack, service *common.Service, proxyApiKey string) *Spider {
	return &Spider{
		slackBot: slackBot,
		proxy:    NewProxy(service.Redis.Master, proxyApiKey),
		service:  service,
	}
}
...
func (this *Spider) GetGzhArticles(wechatName string) ([]Article, error) {
	...
	for _, ret := range articlesResult {
		...
		if ret.Article.Title != "" {
			...
			ids = append(ids, fmt.Sprintf("%d", articleId))
		}
	...
	for _, a := range articles {
		...
		mk, err := this.getArticle(a.Url, profile)
		if err != nil || mk == "" {
			continue
		}
		a.Markdown = mk
...
func (this *Spider) unlockSogouVerify(referrer string, image []byte) (snuid string, err error) {
	...
	for {
		...
		case code := <-codeCh:
			...
			resp, err := grequests.Post("http://weixin.sogou.com/antispider/thank.php", ro)
			if err != nil {
				return "", err
			}
			if resp.Ok {
...
func (this *Spider) unlockWxVerify(link string, image []byte) error {
	...
	for {
		...
		case code := <-codeCh:
			...
			resp, err := grequests.Post("https://mp.weixin.qq.com/mp/verifycode", ro)
			if err != nil {
				return err
			}
			if resp.Ok {
